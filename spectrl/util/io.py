...
def generate_video(env, policy, itno, folder, max_step=10000, stateful_policy=False):
    ...
    video = cv2.VideoWriter(
        video_name, cv2.VideoWriter_fourcc(*'XVID'), 13, (width, height))
...
def extract_plot_data_rm(folder, x_column_num, y_column_num, low, up, csv=True):
    '''
    Load and parse log_info to generate error bars

    Parameters:
        folder: string (name of folder)
        x_column_num: int (column number in log.npy to use)
        y_column_num: int (column number in log.npy to use)
        l: int (lower limit on run number)
        u: int (upper limit on run number)
    ...
    y_log_infos = []
    for itno in range(low, up):
        x_log_info = np.transpose(load_log_info(
            itno, folder, csv=csv))[x_column_num]
        y_log_info = np.transpose(load_log_info(
            itno, folder, csv=csv))[y_column_num]

        x_log_info = np.append([0], x_log_info[1:])
        y_log_info = np.append([0], y_log_info[1:])

        x_val, y_val = _flatten(x_log_info, y_log_info, interval)
        # print(x_log_info[-1], x_val[-1], len(x_val), len(y_val))
        if max_len < len(x_val):
            max_len = len(x_val)
            x_final = x_val

        y_log_infos.append(y_val)

    y_log_infos = [_extend(log_info, max_len) for log_info in y_log_infos]
    data = np.array(y_log_infos)
    curve = np.mean(data, axis=0)
    std = np.std(data, axis=0)
    max_curve = np.amax(data, axis=0)
    return x_final, curve, (curve - std), (curve + std), max_curve
