...
class MultiActor:
    ...
    def __init__(self, stateDims, actionDims, hiddenDim, actionBounds):
        self.n = len(stateDims)
        self.actors = [Actor(stateDims[i], actionDims[i], hiddenDim, actionBounds[i])
                       for i in range(self.n)]

    def forward(self, state):
        return [a.forward(s) for a, s in zip(self.actors, state)]

    def get_action(self, state, using_tensors=False):
        return [a.get_action(s, using_tensors) for a, s in zip(self.actors, state)]
    ...
    def set_use_cpu(self):
        return [a.set_use_cpu() for a in self.actors]
    ...
    def cuda(self):
        self.actors = [a.cuda() for a in self.actors]
        return self
